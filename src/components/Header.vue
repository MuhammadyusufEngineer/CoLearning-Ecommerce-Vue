<template>
    <header>
        <!-- Responsive menu -->
        <div class="flex md:hidden w-full min-h-screen fixed top-0 left-[-100%] justify-center items-center bg-white z-50 duration-300 mobile-menu">
            <div class="flex flex-col min-h-full">
                <div class="flex flex-col items-center gap-[33px]">
                    <a :class="item.class" :href="item.href" v-for="item in anchor1" :key="item.id"> {{ item.text }}</a>

                </div>
            </div>
        </div>
        <!-- responsive nav -->
        <div class="flex flex-row fixed top-0 left-0 w-full bg-white z-50 px-5 md:hidden shadow-lg">
            <div class="logo mr-2">
                <img src="../assets/img/Brand.svg" alt="brand logo">
            </div>
            <div class="mobile-burger relative z-50 right-0 w-5 h-5 bg-cover bg-no-repeat bg-center">
            </div>
        </div>
        <!-- Nav -->
        <div class="main-container hidden md:flex flex-row items-center py-4">
            ...
            <div class="flex items-center gap-[33px]">
                <a :class="item.class" :href="item.href" v-for="item in anchor2" :key="item.id"> {{ item.text }}</a>
            </div>
        </div>
        <!-- Divider -->
        ...
        <div class="main-container flex-row items-center pt-20 md:pt-10 pb-12">
            ...
            <div class="flex items-center justify-between rounded-[12px] max-w-[500px] sm:w-[500px] border border-solid border-[#d1d1d1] bg-[#f9f9f9] py-[10px] px-4">
            ...
            <div class="fixed sm:relative bottom-0 left-0 w-full h-14 sm:py-0 bg-white flex items-center justify-around sm:justify-between sm:w-[88px] ml-0 sm:ml-5 z-40">
                <div>
                ...
                <div class="relative cart-wrapper">
                    ...
                    <div class="absolute -bottom-[8px] -left-[5px] w-4 h-4 rounded-full flex items-center justify-center bg-[#E5704B]">
        ...
        <nav class="bg-[#f9f9f9] py-4">
            <div class="main-container flex-row items-center justify-start gap-[42px]">
                <CustomSelect class="hidden md:block custom-select w-[120px]" v-for="n in 7" :key="n.index" />

                <swiper class="navSwiper block md:hidden"
                 :slides-per-view = "3"
                 :space-between = "50"
                 :breakpoints = "{
                    0: {
                 ...
                 @slideChange="onSlideChange">
                    <swiper-slide v-for="item in navSwiper" :key="item.id">
                        <a href="./products.html" target="_blank">
                            ...
                            <p class="text-[12px] w-20 text-center overflow-hidden text-ellipsis whitespace-nowrap mt-1">
                                {{ item.title }}
                            </p>
                        </a>
                    </swiper-slide>
